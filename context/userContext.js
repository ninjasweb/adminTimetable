...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (user) {
      const querySnapshot = collection(
        db,
        "evento",
        `/${queryPrevId}/stages/${queryId}/artist`
      )
      const unsuscribe = onSnapshot(querySnapshot, (res) => {
  ...
  useEffect(() => {
    if (user) {
      const collectionRef = collection(db, "eventInfo")
      const queryRef = query(collectionRef)
      const unsuscribe = onSnapshot(queryRef, (res) => {
        setEventInfo(res.docs.map((doc) => doc.data()))
      })
      return unsuscribe
    }
  }, [user])
  ...
  // Update Event Info
  const updateEventInfo = async (name, date) => {
    try {
      const collectionRef = collection(db, "eventInfo")
      const docRef = doc(collectionRef, "QLErRaLZFiQQTZubLDW1")
